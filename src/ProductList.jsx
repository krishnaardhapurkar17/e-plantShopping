...
function ProductList({ onHomeClick }) {
  ...
  const [showPlants, setShowPlants] = useState(true); // show plant categories by default
  ...
  const plantsArray = [
    ...
    const handleHomeClick = (e) => {
      ...
      // Track which items are added (sync with cart state)
      React.useEffect(() => {
        const newAdded = {};
        cart.forEach(item => { newAdded[item.name] = true; });
        setAddedToCart(newAdded);
      }, [cart]);
    
      // Calculate total quantity for navbar/cart icon
  ...
  <nav style={{
    ...
    display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '20px'
  }}>
    <ul style={{
      display: 'flex', listStyle: 'none', margin: 0, padding: 0, alignItems: 'center'
    }}>
          ...
          <div className="product-grid" style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
            {category.plants.map((plant, plantIndex) => (
              <div className="product-card" key={plantIndex} style={{
                ...
                padding: '15px', width: '220px', textAlign: 'center', background: '#fdfdfd'
              }}>
                <img
